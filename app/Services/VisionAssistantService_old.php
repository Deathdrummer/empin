...
class VisionAssistantService
{
	...
	/**
	 * Включить строгий режим: только file_search
	 */
	...
	public function ask(?string $prompt = null, UploadedFile|string|array|null $files = null, bool $saveThread = false): string
	{
		...
		$cachedFiles = Cache::get(self::FILE_MAP_CACHE, []);
		if (empty($cachedFiles)) {
			return 'Нет контекстных файлов для анализа';
		}
	...
	private function syncReferenceFiles(): void
	{
		...
		foreach ($this->disk->files($this->dir) as $path) {
			...
			if (!$entry || $entry['hash'] !== $hash) {
				...
				$entry = ['id' => $fileId, 'hash' => $hash, 'mime' => $mime, 'cat' => $cat, 'name' => $filename];
				Cache::forget(self::THREAD_CACHE);
			}
	...
	private function bootThread(): string
	{
		return Cache::rememberForever(self::THREAD_CACHE, function () {
			...
			$fileMap = $this->buildFileMapContext($entries);
			if ($fileMap) {
				$context .= ($context ? "\n\n" : '') . $fileMap;
			}
			...
			if ($other || $context !== '') {
				$attachments = [];
				foreach ($other as $e) {
					$attachments[] = ['id' => $e['id'], 'mime' => $e['mime'], 'cat' => $e['cat']];
				}
	...
	/**
	 * @param array<int, array{0|string,1|string,2|string}|array{id:string,mime:string,cat:string}|array> $attachments
	 */
	private function attachOther(string $threadId, string $text, array $attachments): void
	{
		...
		foreach ($attachments as $att) {
			if (is_array($att) && isset($att['id'])) {
				$fileId = $att['id'];
				$mime = $att['mime'];
				$cat = $att['cat'] ?? 'other';
			} else {
				[$fileId, $mime, $cat] = array_pad($att, 3, 'other');
			}
			...
			if (str_starts_with($mime, 'image/')) {
				...
				$tools = match($cat) {
					'vector' => [['type' => 'file_search']],
					'code' => [['type' => 'code_interpreter']],
					default => [['type' => 'file_search']]
				};
				$attachArr[] = ['file_id' => $fileId, 'tools' => $tools];
			}
		}
	...
	private function buildFileMapContext(array $entries): string
	{
		if (empty($entries)) {
			return '';
		}

		$fileNames = collect($entries)->pluck('name')->filter()->toArray();
		$dbFiles = \App\Models\AssistentFile::whereIn('filename_sys', $fileNames)->get()->keyBy('filename_sys');

		$lines = ["ДОСТУПНЫЕ ФАЙЛЫ:"];
		foreach ($entries as $entry) {
			$sysName = $entry['name'] ?? null;
			if (!$sysName) continue;

			$dbFile = $dbFiles->get($sysName);
			$origName = $dbFile->filename_orig ?? $sysName;

			$lines[] = "- {$origName} (категория: {$entry['cat']})";
		}
		...
		foreach ($list as $msg) {
			...
			foreach ($msg->content as $part) {
				if ($part->type === 'text' && trim($part->text->value) !== '') {
					$text = trim($part->text->value);

					// Удаляем citation markers от OpenAI
	...
	private function ensureVectorStore(array $fileIds): void
	{
		...
		if (!$vsId) {
			...
			do {
				sleep(2);
				$waited += 2;
				$vsStatus = $this->openai->vectorStores()->retrieve($vsId);
				$status = $vsStatus->status ?? 'unknown';
			} while ($status !== 'completed' && $waited < $maxWait);
