...
trait HasCrudController {
	...
	private function _getLastSortIndex($model = null, $sortField) {
		if (!$model) return false;
		$ModelClass = app()->make($model);
		return $ModelClass->max($sortField) ?: 0;
	}
	 ...
	 *		'key'		=> 'id', [:single - если нужно передать скалярное значение]
	 *		'value'		=> 'title',
	 *		'filter'	=> 'id:3'
	 * ] 
	 * @param mixed $setting
